...
class IP {
	 ...
	 * @return string
	 */
	private static function decbin32( $dec ) : string {
	 ...
	 * @return bool
	 */
	public static function ipv4_in_range( $ip, $range ) : bool {
	...
	/**
	 * Converts IP to Long IP
	 * @param string $ip
	 * @return string
	 */
	private static function ip2long6( $ip ) : string {
		if ( substr_count( $ip, '::' ) ) {
	 ...
	 * @return string
	 */
	public static function get_ipv6_full( $ip ) : string {
		$pieces      = explode( "/", $ip, 2 );
		$left_piece  = $pieces[0];
		$right_piece = null;
		if ( count( $pieces ) > 1 ) {
	 ...
	 * @param string $ip is the IPV6 address in decimal format to check if its within the IP range created by the cloudflare IPV6 address, $range_ip.
	 * @param string $range_ip converted to full IPV6 format.
	 *
	 * @return bool returns true if the IPV6 address, $ip,  is within the range from $range_ip.  False otherwise.
	 */
	public static function ipv6_in_range( $ip, $range_ip ): bool {
